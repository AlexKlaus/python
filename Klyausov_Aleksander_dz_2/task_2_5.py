...
for p in sorted(prices):
    ...
    if n == len(prices)-1:  # если идет последний элемент списка то записываем без запятой
        ...
        prices_str += f'{price[0]} руб {price[1]} коп, '  # заполняем строку в нужном формате
    reverce_price.insert(0, f'{price[0]} руб {price[1]} коп')  # сразу создаем реверсирвованый список в нужном формате
    n += 1

print("Цены по возрастанию: ", prices_str)
print("Цены по убыванию: ", ', '.join(reverce_price))
print("Пять самых дорогих товаров: ", ', '.join(reverce_price[:5]))
